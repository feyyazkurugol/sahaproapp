...
/**
 * ✅ IMPORTANT:
 * Backend DTO'ları tenantId/userId istemiyor (claim'den alıyor).
 * Ancak UI şimdilik bu alanları yolluyor olabilir.
 * Bu yüzden tiplerde optional tutuyoruz ama body'ye SADECE backend DTO alanlarını yazıyoruz.
 */
type StartBody = {
  tenantId?: string; // legacy/ignored
  userId?: string; // legacy/ignored
  lat?: number | null;
  lng?: number | null;
};
...
// ✅ Start: body'de sadece { lat, lng } gönder
...
// ✅ Complete: body'de sadece { lat, lng } gönder
...
// ✅ AddPayment: body'de sadece { amount, currency, method } gönder
...
// ✅ UploadPhoto: backend query'de sadece kind istiyor. tenantId/userId query'den kaldırıldı.
...
export async function deleteWorkOrderPhoto(
  ...
  // Not: backend tenant'i claim'den alıyor. Query param fazlalığı sorun değil.
  const url = u(
...
// ✅ Cancel: body'de sadece { reason, note, lat, lng } gönder
