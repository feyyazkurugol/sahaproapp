...
function statusUiMeta(key: StatusKey) {
  if (key === "pending")
    return { cls: "bg-blue-50 text-blue-700 ring-blue-200", dot: "bg-blue-500" };
  if (key === "in_progress")
    return { cls: "bg-amber-50 text-amber-800 ring-amber-200", dot: "bg-amber-500" };
  if (key === "completed")
...
async function getCurrentLocation(timeoutMs = 12000): Promise<GeoResult> {
  ...
  return await new Promise<GeoResult>((resolve) => {
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        if ((err as any)?.code === 1) return resolve({ ok: false, messageKey: "geo.permission_denied" });
        if ((err as any)?.code === 2) return resolve({ ok: false, messageKey: "geo.unavailable" });
        if ((err as any)?.code === 3) return resolve({ ok: false, messageKey: "geo.timeout" });
...
export default function JobDetailClient({ initial, workOrderId }: Props) {
  ...
  // ✅ only for pages that still need tenantId (detail/delete photo)
  function requireTenantId() {
    if (!tenantId) throw new Error("tenant_required");
    return tenantId;
  }

  // ✅ only for upload (legacy UI expectation)
  function requireUploadIds() {
    if (!tenantId) throw new Error("tenant_required");
  ...
  async function onStart() {
    try {
      ...
      // ✅ Backend DTO: only {lat,lng}
      await startWorkOrder(workOrderId, { lat, lng });
  ...
  async function onComplete() {
    try {
      ...
      // ✅ Backend DTO: only {lat,lng}
      await completeWorkOrder(workOrderId, { lat, lng });
  ...
  async function onCancel() {
    ...
    try {
      ...
      // ✅ Backend DTO: {reason, note?, lat, lng}
      await cancelWorkOrder(workOrderId, { reason, lat, lng });
  ...
  async function onUpload(kind: "before" | "after", file: File | null) {
    ...
    try {
      ...
      const { tenantId, userId } = requireUploadIds();
  ...
  async function onDeletePhoto(attachmentId: string) {
    ...
    try {
      ...
      await deleteWorkOrderPhoto(workOrderId, tid, attachmentId);
  ...
  async function onPay() {
    try {
      ...
      // ✅ Backend DTO: {amount,currency,method}
        ...
        <div className="p-4">
          ...
          {isCancelled && (wo.cancelReason || wo.cancelNote) ? (
            <div className="mt-3 rounded-2xl bg-red-50 border border-red-200 p-3">
              ...
              {wo.cancelNote ? (
                <div className="mt-1 text-xs text-red-800 opacity-80">{wo.cancelNote}</div>
              ) : null}

              {wo.cancelledAt ? (
                <div className="mt-1 text-[11px] text-red-600">{fmt(wo.cancelledAt)}</div>
              ) : null}
            </div>
          ) : null}
        ...
        <div className="p-4">
          ...
          <div className="mt-3 grid grid-cols-1 gap-2">
            ...
            {!canStart && !canComplete && !canCancel ? (
              <div className="rounded-2xl bg-gray-50 border border-black/5 p-3 text-xs text-gray-600">
                {t("job.actions.disabled")}
              </div>
            ) : null}
          </div>
        </div>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {canAddPhoto ? (
              ...
              <div className="text-[11px] text-gray-500">
                {isLocked ? t("job.photos.locked") : t("job.photos.need_in_progress")}
              </div>
            )}
          </div>
                ...
                {photosBefore.length ? (
                  photosBefore.map((p) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className="relative overflow-hidden rounded-2xl border border-black/10 bg-gray-50"
                        title={fmt(p.takenAt)}
                      >
